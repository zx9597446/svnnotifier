...
namespace CHD.SVN_Notifier
{
	public partial class MainForm : Form
	{
		...
		private void FormInit()
		{
			...
			{
			    var item = new ListViewItem(folder.Path, imageIndex_Unknown) {ToolTipText = folder.Path};
		...
		private void UpdateTray (bool newNonUpdatedFoldersChanged)
		{
			...
			if (newNonUpdatedFoldersChanged)
			{
				...
				if (newNonUpdatedFolders.Count > 0)
				{
					...
					notifyIcon.ShowBalloonTip (Config.ShowBalloonInterval, "Update needed, doing update now...", balloonMessage, ToolTipIcon.Info);
                    UpdateAll();
				}
			}
		}
